...
SampleFSIO::SampleFSIO(unsigned int *extmemArray, long extmemSize,
                       Screen *screen) {
    ...
    // initialize waveFormBuffer for all 72 possible samples -> ToDo: find a
    // more progressive data structure? waveFormBuffer for recorder is stored at
    // position 72
    for (int s = 0; s < 73; s++) {
...
char *SampleFSIO::getSongPath() { return _songPath; }
...
void SampleFSIO::writeAllSamplesToWaveformBuffer() {
    ...
    // generate waveform buffer for last recording (RECORD.RAW, saved at
    // position 72 -> bank0 3, sample0 0)
...
void SampleFSIO::generateWaveFormBufferForSample(byte bank0, byte sampleId0) {
    int heightDivisor =
        floor(32767 / 60);  // assumes the display area is 120px (2*60px) high
    File sample;
    ...
    char *filename = (bufferPosition == 72 ? recorderFilename
                                           : sampleFilename[bank0][sampleId0]);
    ...
    // defines how many samples will be aggregated to one pixel - used for
    // cropping markers to get the right positions to cut from the original RAW
    // file
    pixelToWaveformSamples[bufferPosition] =
        (pxIterations == 0 ? 1 : pxIterations);
    ...
    if (pxIterations > 1) {
        for (int pi = 0; pi < 320; pi++) {
            ...
            waveFormBuffer[bufferPosition][pi][0] =
                abs(floor(tempMin / heightDivisor));
            waveFormBuffer[bufferPosition][pi][1] =
                abs(floor(tempMax / heightDivisor));
        }
    } else {
        ...
        while (sample.available()) {
            ...
            if (peak >= 0) {
                waveFormBuffer[bufferPosition][i][0] = 0;
                waveFormBuffer[bufferPosition][i][1] =
                    abs(floor(peak / heightDivisor));
            } else {
                waveFormBuffer[bufferPosition][i][0] =
                    abs(floor(peak / heightDivisor));
                waveFormBuffer[bufferPosition][i][1] = 0;
            }
            i++;
...
boolean SampleFSIO::copyFilePart(const char *f1, const char *f2, long byteStart,
    ...
    if (byteEnd == 0 || byteEnd > fromSize) {
        byteEnd = fromSize;
    }

    if (fromSize < byteStart || byteStart >= byteEnd) {
        Serial.println(
            "operation not possible as we cannot shift back in time");
        return false;
    }
    ...
    while (from.available() > 0 && byteStart + offset <= byteEnd) {
        ...
        if ((tempBuff > 0 && intBuff <= 0) || (tempBuff < 0 && intBuff >= 0) ||
            (!firstCutFound && intBuff == 0)) {
            // find first zero cut
            if (!firstCutFound) {
...
boolean SampleFSIO::addSampleToMemory(byte bank1, byte sampleId1,
                                      boolean forceReload) {
    ...
    if (_sampleOffsets[sample72] == -1 || forceReload) {
        ...
        _nextOffset = copyRawFromSdToMemory(
            sampleFilename[bank1 - 1][sampleId1 - 1], offset);
...
void SampleFSIO::removeSampleFromMemory(byte sampleId) {
    ...
    for (int i = 0; i < 72; i++) {
        if (_sampleOffsets[i] > sampleOffsetToRemove &&
            _sampleOffsets[i] < sampleOffsetToRemove &&
            _sampleOffsets[i] < nextOffset) {
            nextOffsetId = i;
        }
    }
    ...
    if (nextOffsetId < 72) {
        ...
        // update all other _sampleOffsets which are larger than the offset to
        // remove
        ...
        // no other offset? -> sample to remove is the last one in extmem. set
        // _nextOffset to the current position
...
byte SampleFSIO::getExtmemUsagePercent() {
    float f = (static_cast<float>(_nextOffset) /
               static_cast<float>(_extmemSize / 4)) *
              100.0;
    return round(f);
};
...
boolean SampleFSIO::loadSampleInfosFromSD() {
    ...
    if (SD.exists(buff)) {
        ...
        for (uint16_t i = 0;
             i < sizeof(_sampleInfos) / sizeof(sampleInfosStruct); i++) {
...
boolean SampleFSIO::saveSampleInfosToSD() {
    ...
    for (uint16_t i = 0; i < sizeof(_sampleInfos) / sizeof(sampleInfosStruct);
         i++) {
        bufferBlocks = (byte *)&_sampleInfos[i];
...
void SampleFSIO::setSampleInfosName(int sampleId1, String name) {
    ...
    if (name.length() >= 39) {
        ...
        tempString.append(" ... ");
        // get end of filename and remove suffix .raw from filename (quich and
        // dirty four chars cut)
        tempString.append(
            name.substring(name.length() - 14, name.length() - 4));
    } else {
        tempString = name;
    }

    tempString.toCharArray(_sampleInfos[sampleId1 - 1].name,
                           tempString.length() + 1);
    saveSampleInfosToSD();
};
...
void SampleFSIO::getSampleInfosName(int sampleId1, int maxLength,
                                    char *returnArray) {
